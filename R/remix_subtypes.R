...
#' deconstructSigs_cols <- hcols_to_ds_cols(subtypes_in)
#'}
#' @export
...
#' mu_counts <- read.table("/path/to/subtype_count_matrix.txt", header=T, stringsAsFactors=F)
#' subtypes_in <- colnames(mu_counts)[-1]
#' SomaticSigs_rows <- hcols_to_ss_rows(subtypes_in)
#'}
#' @export
...
#' mu_counts <- read.table("/path/to/subtype_count_matrix.txt", header=T, stringsAsFactors=F)
#' subtypes_in <- colnames(mu_counts)[-1]
#' YAPSA_rows <- hcols_to_yapsa_rows(subtypes_in)
#'}
#' @export
hcols_to_yapsa_rows <- function(subtypes){
  subtypes_short <- gsub("_", ">", subtypes)
